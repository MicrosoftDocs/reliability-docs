...
A load balancer can be either public or internal. A public load balancer accepts traffic from the internet through a public IP address resource. An internal load balancer accepts traffic only within your virtual network and other networks that you connect to the virtual network.
...
For more information about Load Balancer, see [Load Balancer components](/azure/load-balancer/components).
...
:::image type="complex" border="false" source="./media/reliability-load-balancer/zone-redundant-public-load-balancer.svg" alt-text="Diagram that shows a zone-redundant public load balancer with a zone-redundant public IP address that directs traffic to three VMs in different availability zones." lightbox="./media/reliability-load-balancer/zone-redundant-public-load-balancer.svg":::
   Architecture diagram that shows a zone-redundant public load balancer deployed across three availability zones. The zones appear as blue rectangles and the load balancer icon appears in green. A zone-redundant public IP address connects to the zone-redundant front-end IP configuration. The public load balancer section includes a zone-redundant front-end IP configuration, a load-balancing rule, and a back-end pool.
:::image-end:::
...
:::image type="complex" border="false" source="./media/reliability-load-balancer/zone-redundant-internal-load-balancer.svg" alt-text="Diagram that shows a zone-redundant internal load balancer with a zone-redundant public IP address that directs traffic to three VMs in different availability zones." lightbox="./media/reliability-load-balancer/zone-redundant-internal-load-balancer.svg":::
   Architecture diagram that shows a zone-redundant internal load balancer deployed across three availability zones. The zones appear as blue rectangles and the load balancer icon appears in green. The load balancer includes a zone-redundant front-end IP configuration, a load balancing rule, and a back-end pool that spans all three zones. Traffic routes from the internal front-end IP configuration through the load balancer to the VMs across all zones.
:::image-end:::

> [!NOTE]
> You can deploy zonal load balancers, but we recommend that you use zone-redundant load balancers for all workloads, including workloads that you deploy into a single zone. Microsoft currently migrates all public IP addresses and load balancers to zone-redundant configurations.

In regions without availability zones, you create load balancers in a *nonzonal* or *regional* configuration by using a front-end configuration with no zone configured. If the region experiences an outage, your nonzonal load balancers might experience downtime.
...
If you choose to deploy your VMs in the same availability zone, you can deploy a zone‑redundant front-end IP configuration on your load balancer, as the following diagram shows.
...
:::image type="complex" border="false" source="./media/reliability-load-balancer/zone-redundant-load-balancer-zonal-virtual-machines.svg" alt-text="Diagram that shows a zone-redundant public load balancer directing traffic to two different VMs in zone 1." lightbox="./media/reliability-load-balancer/zone-redundant-load-balancer-zonal-virtual-machines.svg":::
   Architecture diagram that shows a zone-redundant public load balancer deployed across three availability zones, but with both VMs located only in zone 1. The zones appear as blue rectangles and the load balancer icon appears in green. A zone-redundant public IP address connects to a zone-redundant front-end IP configuration. The load balancer includes a zone-redundant front-end IP configuration, load-balancing rule, and a back-end pool that spans all three zones. The back-end pool contains two VMs that are both deployed in zone 1 only, and zone 2 and zone 3 have no VMs. This configuration creates a single point of failure (SPoF) because all VMs reside in one zone.
:::image-end:::
...
- **Change the availability zone configuration of an existing load balancer.** To change the availability zone configuration of an existing load balancer, replace the front-end IP configuration. You can use this approach to move from a zonal configuration to a zone-redundant front-end IP configuration. Consider the high-level approach:
...
- **Data replication between zones:** Load Balancer operates as a network pass-through service that doesn't store or replicate application data. Even if you configure [session persistence](/azure/load-balancer/distribution-mode-concepts#session-persistence) on the load balancer, the load balancer stores no state. Session persistence adjusts the hashing process to route requests to the same back-end instance, but the load balancer doesn't guarantee session persistence. When the back-end pool changes, the load balancer recomputes the distribution of client requests and doesn't store or sync state.
...
If the zone failure also affects your compute services in that zone, then as back-end instances in the recovered zone pass health checks, Load Balancer automatically adds them back to the healthy back-end pool. Traffic distribution rebalances across all available zones based on the load balancing algorithm and back-end instances' health status.

### Test for zone failures

The Azure platform manages traffic routing, zone-down response, and recovery. Azure fully manages these capabilities, so you don't need to initiate or validate availability zone failure processes.
...
You deploy public and internal load balancers into a single Azure region. If the region becomes unavailable, your load balancers in that region also become unavailable. Load Balancer provides native multi-region support through its cross-region load balancer, which supports load balancing across Azure regions. You can also use other Azure load-balancing services to route and fail over traffic across regions.
