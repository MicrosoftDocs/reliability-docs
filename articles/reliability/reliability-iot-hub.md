...
When a region supports both of these types of zone redundant, critical service data - including the Device Identity Registry - is synchronously replicated across availability zones within the region. As a result, in the event of a single-zone failure, no data loss is expected, and the service automatically reroutes device connectivity and message traffic to a healthy zone. While in-flight requests may be transiently impacted during the failover event, service continuity is maintained and device retry logic typically ensures recovery.

### Requirements

**Region support:** The type of availability zone support for your IoT hub depends on the region that it's deployed in.
...
If your IoT hub is in a nonpaired region, or if the default replication and failover behavior doesn't meet your needs, you design and implement a [custom multiregion failover strategy](#custom-multiregion-solutions-for-resiliency), including the following steps:
...
If your IoT hub is in a nonpaired region, see [Custom multiregion solutions for resiliency](#custom-multiregion-solutions-for-resiliency) for possible cost information.
...
If your IoT hub is in a nonpaired region, you need to plan your own cross-region replication and failover approach. For more information, see [Custom multiregion solutions for resiliency](#custom-multiregion-solutions-for-resiliency).
...
- **Data replication between regions:** Data, including the Device Identity Registry, is replicated automatically to the paired region. Replication occurs asynchronously, which means that some data loss is expected if a failover occurs. There's no data replication between regions for IoT hubs in nonpaired regions.
...
- **Merge logic:** When the primary region becomes available again, all the state and data created in the secondary region must be migrated back to the primary region. This state and data mostly relate to device identities and application metadata, which must be merged with the primary IoT hub and any other application-specific stores in the primary region.
  ...
  To simplify this step, use *idempotent* operations. Idempotent operations minimize the side effects from the eventual consistent distribution of events and from duplicates or out-of-order delivery of events. Also, the application logic should be designed to tolerate potential inconsistencies or slightly out-of-date state. This scenario can occur because of the extra time that it takes for the system to heal based on RPOs. For implementation guidance, see the [Idempotent Consumer pattern](/azure/architecture/patterns/idempotent-consumer).
