...
- **Backup storage:** If the server is deployed in a region with availability zones, backups are stored in zone-redundant storage (ZRS), regardless of the server's high availability configuration. For servers deployed in regions without availability zones, backups are stored in locally redundant storage (LRS).
