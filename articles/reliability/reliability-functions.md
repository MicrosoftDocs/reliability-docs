...
- **Enable zone redundancy on an existing plan:** You can enable or disable availability zones on existing Elastic Premium plans using the Azure CLI. See [Enable zone redundancy on an existing plan](#enable-zone-redundancy-on-an-existing-plan) for details about Elastic Premium-specific capacity behavior.
...
>
> When enabling zone redundancy, you must update both the plan-level `zoneRedundant` property to `true` and `sku.capacity` to `2`, and the app-level `minimumElasticInstanceCount` to at least 2 on each function app that you want to be zone-redundant. Setting only the plan update command doesn't enforce a minimum of 2 instances.

#### [Azure portal](#tab/azure-portal)

The Azure portal doesn't support toggling availability zone redundancy on existing Elastic Premium plans yet. Use the **Azure CLI** tab for the current supported workflow.
...
1. Enable zone redundancy on the plan:
    ...
    > [!NOTE]
    > The `sku.capacity=2` parameter in this command sets the intended minimum, but the minimum isn't enforced until you complete step 2.

1. Update always-ready instances to at least 2 for each function app in the plan:
...
After disabling zone redundancy, you can optionally reduce `minimumElasticInstanceCount` back to 1 on your function apps.

#### [Bicep template](#tab/bicep)

The following example shows how to enable zone redundancy on an existing Elastic Premium plan using Bicep:
...
> [!NOTE]
> Update the `minimumElasticInstanceCount` property to at least 2 on all function apps in the plan to ensure zone redundancy requirements are met.

#### [ARM template](#tab/arm-template)

The following example shows how to enable zone redundancy on an existing Elastic Premium plan using an ARM template:
...
In the Azure portal, navigate to your function app. Under **Settings**, select **Instances**. The **Instances** page shows each running instance and the availability zone it's placed in.

To query instance zone placement using the Azure CLI:
...
Replace `<RESOURCE_GROUP>` and `<APP_NAME>` with the names of your resource group and function app.
...
- `physicalZone` shows the actual availability zone the instance is placed in, in the format `{region}-az{N}`.
- For a zone-redundant plan with 2 or more instances, you see instances distributed across different zones.

### Troubleshooting

If zone redundancy isn't working as expected after following these steps, review the [Common Issues and Solutions](https://techcommunity.microsoft.com/blog/appsonazureblog/deep-dive-on-availability-zones-in-azure-app-service/4433526) section in the deep dive blog post on Availability Zones in Azure App Service. While the blog post focuses on App Service plans, many troubleshooting steps also apply to Elastic Premium plans.
