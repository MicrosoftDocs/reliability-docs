...
- **Migrate existing VMs to a zonal configuration** You can migrate from a nonzonal VM to a zonal VM. This process requires shutting down the virtual machine, updating the zone properties and starting it again. The feature is current in preview. For more information, see [migrate to availability zones (preview)](/azure/virtual-machines/migrate-to-availability-zone).
