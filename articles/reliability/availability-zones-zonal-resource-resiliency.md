...
The following diagram shows one example architecture that uses three availability zones. Some Azure regions provide more than three availability zones, and a workload or service might use only a subset of the zones available in the region.
...
:::image type="complex" source="./media/availability-zones-zonal-reliability/zonal-zone-redundant.svg" border="false" lightbox="./media/availability-zones-zonal-reliability/zonal-zone-redundant.svg" alt-text="Diagram that shows a solution that includes both zonal VMs and zone-redundant components.":::
    The image shows an example architecture that uses three availability zones. A zone-redundant web application and a zone-redundant load balancer span the zones shown. Each zone contains a database replica and a zonal virtual machine (VM). A dotted arrow labeled customer-configured replication and failover connects the three replicas.
:::image-end:::

The three database replicas in this example are customer-configured workload components. Their count doesn't have to match either the number of zones in the region or the number of zones used by the zone-redundant services.
